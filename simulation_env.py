...
class Simulation:
    def __init__(self, mpc_controller, mpc_param):
        self.mpc_param = mpc_param
        self.q = 1
        self.mpc_controller = mpc_controller(self.mpc_param)
        self.T = self.mpc_param.T
        self.steps_N = self.mpc_param.N
        self.simulate()
...
if __name__ == '__main__':
    ...
    if train:
        ...
        obs = env.reset()
        while True:
            action, _states = model.predict(obs.reshape(1, 4*(2+1)), deterministic=True)
            action = np.array([1])
