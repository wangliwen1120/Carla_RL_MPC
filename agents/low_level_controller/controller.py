...
class VehiclePIDController:
    ...
    def run_step_acc_2_wp(self, target_acc, current_acc, waypoint1, waypoint2):
        """
        Execute one step of control invoking both lateral and longitudinal PID controllers to reach a target waypoint
        at a given target_acc.

        :param target_acc: desired vehicle acc
        :param waypoint: target location encoded as a waypoint
        :return: distance (in meters) to the waypoint
        """
        throttle, current_acc = self._lon_controller.run_step_acc(target_acc, current_acc)
...
class PIDLongitudinalController:
    ...
    def __init__(self, vehicle, K_P=5.0, K_D=0.0, K_I=0.0):
        """
        :param vehicle: actor to apply to local planner logic onto
    ...
    def run_step_acc(self, target_acc, current_acc):

        return self._pid_acc_control(target_acc, current_acc), current_acc
    ...
    def _pid_control(self, target_speed, current_speed):
        ...
        # print((self._K_P * _e) + (self._K_D * _de / self.dt) + (self._K_I * _ie * self.dt))
        return np.clip((self._K_P * _e) + (self._K_D * _de / self.dt) + (self._K_I * _ie * self.dt), 0.0, 1.0) / 1.0

    def _pid_acc_control(self, target_acc, current_acc):
        """
        Estimate the throttle of the vehicle based on the PID equations

        :param target_acc:  target speed in m/s^2
        :param current_acc: current speed of the vehicle in Km/h
        :return: throttle control in the range [0, 1]
        """
        _e = (target_acc - current_acc)
        self._e_buffer.append(_e)
        ...
        # return (np.clip((130 * _e) + (0.5 * _de / self.dt) + (300 * _ie * self.dt), -1000.0, 1000.0) + 1000.0) / 2000
        return (np.clip((130 * _e) + (0.5 * _de / self.dt) + (300 * _ie * self.dt), -250.0, 250.0) + 250.0) / 500
