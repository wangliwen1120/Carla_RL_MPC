...
class MPC_controller_lon_lat_ipopt_nonlinear:
    ...
    def calc_input(self, x_current, x_frenet_current, obj, ref, ref_left, u_last, q, ru, rdu):
        ...
        x_predict = MX(self.x_predict)
        y_predict = MX(self.y_predict)
        phi_predict = MX(self.phi_predict)
        Y_ext = MX(self.Y_ext)

        for i in range(self.Np - 1):
            # 自车状态预测

            idx_c = min(i, self.Nc - 1)

            x_predict[i + 1] = x_predict[i] + U[2 * idx_c] * np.cos(phi_predict[i]) * self.T
            y_predict[i + 1] = y_predict[i] + U[2 * idx_c] * np.sin(phi_predict[i]) * self.T
            phi_predict[i + 1] = phi_predict[i] + U[2 * idx_c] * np.tan(U[2 * idx_c + 1]) * self.T / self.L

        for i in range(self.Np):
            Y_ext[3 * i] = x_predict[i]
        ...
        # phi_predict = self.phi_predict
        # Y_ext = self.Y_ext
        # U = np.ones([self.Np*self.Nc , 1])
        #
        # for i in range(self.Np - 1):
        #     # 自车状态预测
        #
