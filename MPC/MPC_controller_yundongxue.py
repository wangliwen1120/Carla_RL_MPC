...
class MPC_controller_yundongxue:
    ...
    def calc_input(self, x,ref):
        ...
        # self.Matrix_state_new = np.zeros([self.Nx + self.Nu, 1])
        # self.Matrix_state_new[0] = self.x[0] - self.ref[0]  # S
        # self.Matrix_state_new[1] = self.x[1] - self.ref[1]  # D
        # self.Matrix_state_new[2] = self.x[2] - self.ref[2]  # PHI
        # self.Matrix_state_new[3] = self.U[0]
        # self.Matrix_state_new[4] = self.U[1]
        #
        # self.Matrix_Ak = np.zeros([self.Nx, self.Nx])
        # self.Matrix_Ak[0, 1] = self.ref[3,0] * self.ref[4,0] * np.cos(self.ref[2,0]) / (1 - self.ref[3] * self.ref[1])**2
        # self.Matrix_Ak[0, 2] = self.ref[4,0] * np.sin(self.ref[2,0]) / (self.ref[3] * self.ref[1] - 1)
        # self.Matrix_Ak[1, 2] = self.ref[4, 0] * np.cos(self.ref[2, 0])
        # self.Matrix_Ak[2, 1] = -(self.ref[3,0]**2) * self.ref[4,0] * np.cos(self.ref[2,0]) / (1 - self.ref[3] * self.ref[1])**2
        # self.Matrix_Ak[2, 2] = self.ref[3,0] * self.ref[4, 0] * np.sin(self.ref[2, 0]) / (self.ref[3] * self.ref[1] - 1)
        #
        # self.Matrix_Bk = np.zeros([self.Nx, self.Nu])
        # self.Matrix_Bk[0, 0] = np.cos(self.ref[2,0])/(1-self.ref[3] * self.ref[1])
        # self.Matrix_Bk[1, 0] = np.sin(self.ref[2,0])
        # self.Matrix_Bk[2, 0] = np.tan(self.ref[4,0]) / self.L - self.ref[3] * np.cos(self.ref[2,0])/(1-self.ref[3] * self.ref[1])
        # self.Matrix_Bk[2, 1] = self.ref[3,0] / (self.L * np.cos(self.ref[5,0] ** 2))

        # self.Matrix_state_new[0] = self.x[0] - self.ref[0] +10  # y-ydes
        self.Matrix_state_new[0] = self.x[0] - self.ref[0]
        self.Matrix_state_new[1] = self.x[1] - self.ref[1]  # Vy-Vydes
        self.Matrix_state_new[2] = self.x[2] - self.ref[2]  # phi-phides
        self.Matrix_state_new[2] = self.x[2] - self.ref[2]  # Vphi-Vphides
        self.Matrix_state_new[4] = self.U[1]
        ...
        rankk_A = np.linalg.matrix_rank(self.Matrix_A_cons)
        rankk_P = np.linalg.matrix_rank(self.Matrix_H_QP_du)
        rankk_Matrix_Cy = np.linalg.matrix_rank(self.Matrix_Cy)
        rankk_Matrix_theta = np.linalg.matrix_rank(self.Matrix_theta)
        rankk_Matrix_yita = np.linalg.matrix_rank(self.Matrix_yita)
        rankk_Matrix_Ap = np.linalg.matrix_rank(self.Matrix_Ap)
        rankk_Matrix_B_ext = np.linalg.matrix_rank(self.Matrix_Bp)
        rankk_Matrix_A_ext = np.linalg.matrix_rank(self.Matrix_Ap)
        rankk_G = np.linalg.matrix_rank(matrix(np.vstack((self.Matrix_A_cons, np.eye(self.Nc * self.Nu + 1), -np.eye(self.Nc * self.Nu + 1)))))
        # # cvxopt求解过程
